...
summary: "TeddyLee의 RAG 구현 난제와 해결책을 다룬 세미나 내용이다. RAG가 실무에서 어려운 이유(문서 품질, 청크 크기, 검색 정확도, 환각 문제), 최적화 전략(하이브리드 검색, Reranking, 프롬프트 튜닝), 평가 방법론, 그리고 langchain-kr 라이브러리를 활용한 실전 구현 사례를 포함한다."
