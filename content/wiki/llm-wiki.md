...
sources:
  ...
  - https://github.com/braincrew-lab/braincrew-wiki
  - https://github.com/braincrew-lab/deep-agent-builder-claw-wiki
  - https://github.com/braincrew-lab/sk-hynix-wiki
created: 2026-05-17
updated: 2026-05-17
author: wiki-curator
draft: false
...
- Braincrew Wiki 구현은 이 패턴을 팀 단위로 확장한다. 사람은 Obsidian 또는 `/bc-wiki-add`로 `content/raw/`에 메모하고, GitHub Action/Anthropic Routine이 raw 변경을 감지해 `content/wiki/` PR을 만든다. pre-commit hook과 path-guard CI로 사람이 `wiki/`를 직접 수정하지 못하게 막아 raw→wiki 단방향 흐름을 강제한다.[^5]
- Braincrew Wiki에서 배운 중요한 구현 디테일은 운영 상태를 공개 콘텐츠와 분리하는 것이다. `content/`는 웹 배포 경계이고, `.wiki-state/log.md`, `.wiki-state/review.md`, `.wiki-state/hashes`는 공개하지 않는다. 이 분리는 로그·검토 항목·hash ledger가 사이트에 노출되는 문제를 구조적으로 방지한다.[^5]
...
- `sk-hynix-wiki`는 컨설팅 지식베이스형 적용 예시다. `PROJECT.md`를 항상 먼저 읽고, `wiki-search`로 근거를 찾은 뒤, 답변 후 새 인사이트를 `wiki-write`로 환원하는 PROJECT-first → Wiki-first 루프를 강제한다.[^7]
...
2. **전용 wiki-curator 스킬 추가**: Braincrew Wiki처럼 `ingest`, `lint`, `reflect`, `migrate`를 분리한다. 이 블로그에서는 우선 `ingest`와 `lint`만 자동화해도 효과가 크다.
...
|-----------|-----------|------------------------|
| `braincrew-wiki` | raw는 사람/Obsidian이 작성, wiki는 Routine이 작성, path-guard CI와 pre-commit hook으로 경계 강제 | `content/wiki/` 직접 수정 방지, raw frontmatter 검증, 루틴 PR 자동 생성 |
...
| `sk-hynix-wiki` | 컨설팅 위키. PROJECT-first → wiki-search → 답변 → wiki-write 루프 | 큰 주제별 `PROJECT.md`/MOC를 두고, 답변 후 새 인사이트를 wiki에 환원 |
...
- [[obsidian-notion-sync]] - Braincrew Wiki처럼 Obsidian을 authoring UI로 쓰면 raw 작성 경험을 유지하면서 Git/웹 배포/LLM 정제를 붙일 수 있다.
...
[^5]: [braincrew-lab/braincrew-wiki](https://github.com/braincrew-lab/braincrew-wiki) — inspected files: `README.md`, `CLAUDE.md`, `docs/02-ai-curation.md`, `.claude/skills/wiki-curator/`
[^6]: [braincrew-lab/deep-agent-builder-claw-wiki](https://github.com/braincrew-lab/deep-agent-builder-claw-wiki) — inspected files: `README.md`, `00-overview.md`, `research-log/`, `proposals/`
[^7]: [braincrew-lab/sk-hynix-wiki](https://github.com/braincrew-lab/sk-hynix-wiki) — inspected files: `CLAUDE.md`, `docs/wiki/PROJECT.md`, `docs/wiki/99_Settings/`
