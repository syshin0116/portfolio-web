...
summary: "[[MinerU - PDF Parser|MinerU]] 2.x는 패키지명(`magic-pdf`→`mineru`), PyMuPDF 제거, VLM/Hybrid 백엔드 추가 등 대규모 변경을 거쳤다. Pipeline 백엔드 기준으로 DocLayout-YOLO(YOLOv10)→YOLOv8 MFD→UniMERNet→SLANET+/UNet→PytorchPaddleOCR 순서로 파싱이 진행된다. Apple Silicon MPS 가속 테스트 결과 CPU 대비 약 37% 빠른 처리 속도를 확인했다."
...
MinerU는 v2.0에서 근본적인 변화를 겪었다. 기능 몇 개가 추가된 정도가 아니라 패키지명, CLI, 의존성, 아키텍처 전반이 바뀌었다.
