...
summary: "검색 품질을 올려도 첫 실행에서 막히거나 출력이 파싱 불가능하면 제품은 실패한다. 이번 작업에서는 AI 에이전트와 인간의 출력 기본값 충돌을 TTY 감지로 해결하고, score 필드를 Tool 스키마에 주입할지 별도 래퍼로 분리할지 결정하고, 빈 결과의 exit code가 0이어야 하는 이유를 정리하고, 자동 다운로드가 편의인지 부작용인지 판단한 과정을 기록한다."
