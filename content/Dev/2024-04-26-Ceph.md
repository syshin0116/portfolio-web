...
summary: "Ceph는 여러 스토리지를 클러스터로 묶어 하나로 보이게 하는 오픈소스 분산 스토리지 플랫폼이다. RADOS 기술을 기반으로 데이터 복제, 자동 복구, 확장성을 제공하며, RBD(Rados Block Device), CephFS(파일 시스템), Object Storage(S3/Swift API) 등 다양한 인터페이스를 지원한다. 분산 저장으로 복구가 용이하고 하나의 클러스터로 접근이 편리하다는 장점이 있다."
