---
title: "요구사항 발굴: AX 프로젝트가 시작부터 막히는 이유"
date: 2026-05-28
tags:
  - AI
  - AX
...
왜 가장 앞선 AI 회사들이, 모델이 이렇게 좋아진 시대에, 사람을 고객사에 *몸으로* 심을까? 답은 단순하다. **요구사항은 여전히 발굴 대상이지 수집 대상이 아니기 때문이다.** 모델이 좋아질수록 오히려 "현장에서 진짜 문제를 캐는" 사람의 가치가 올라간다.
...
2. **합격 기준이 통계치다.** 비결정적 시스템이라 "되거나/안 되거나"가 아니라 "정밀도 0.92, 재현율 0.85"가 합격선이 된다. 그런데 pass/fail로만 생각하는 비전문가 고객에게서 어떻게 허용 오차를 끌어낼 것인가?
...
**3. DX 없이 AX를 바란다.** 디지털 데이터가 있어야 AI를 학습시키고, 프로세스가 디지털화돼 있어야 AI를 현장에 붙인다.[^dxax] BCG의 "10-20-70" 법칙은 이걸 숫자로 말한다. AI 가치의 10%만 알고리즘이고 20%가 기술·데이터, **70%가 사람과 프로세스**다.[^bcg] 고객이 기대하는 "자동화"는 그 10%이고, 나머지 90%(데이터·프로세스 기반)가 없으면 작동하지 않는다. MIT CISR의 기업 AI 성숙도 모델도 같은 결을 보여준다. 데이터 사일로를 통합하는 단계(2단계)를 넘기 전까지는 재무 성과가 업계 평균을 *밑돈다.*[^cisr] RAND가 AI 프로젝트 실패를 조사했을 때도, 데이터 부족은 여러 원인 중 하나였고 **가장 흔한 1차 원인은 "프로젝트 의도의 오해·오소통"**이었다.[^rand] 실패의 뿌리는 모델이 아니라 *앞단*이라는 뜻이다.
...
위에서 LLM 인터뷰 도구를 이야기했지만, 그 도구는 **언제든 교체된다.** GPT-4o가 다음 모델로, 클로드 코드가 Codex로, Codex가 또 다른 무엇으로 바뀐다.

바뀌지 않는 건 무엇인가. **Contextual inquiry, Mom Test, laddering, JTBD는 40년을 버틴 기질(durable substrate)이다.** Palantir의 forward deployed 모델이 정보기관 시절부터 지금 OpenAI·Anthropic까지 살아남은 것도, 그 산출물인 Ontology가 도구가 아니라 *데이터 모델*이기 때문이다. 인터뷰의 산출물 - 배경 → 문제 → 후보 → 이유 → 선택으로 구조화된 결정 레코드 - 도 어떤 도구로 만들었든 마크다운 텍스트로 남는다.

그래서 AX Engineering의 질문은 "어떤 AI 인터뷰 도구를 쓸까"가 아니다. **"어떤 도구로 인터뷰하든, 그 산출물이 도구에 묶이지 않는 기계 판독 가능한 아티팩트로 남는가"**다. 이 질문의 끝이 시리즈 마지막 글 "도구는 소모품이다"의 결론이다.
...
[^dxax]: "DX 없이는 AX도 없다 - 디지털 데이터가 있어야 AI를 학습시킬 수 있고, 프로세스가 디지털화돼 있어야 AI 솔루션을 현장에 적용할 수 있다." [flow.team](https://platform.flow.team/blog/noai-without-dx). SK AX도 ["DX를 넘어 AX로"](https://www.skax.co.kr/insight/trend/3136)를 내세운다.
