...
Docling은 상용 프로젝트에서 부담 없이 쓸 수 있는 오픈소스 PDF 파서다. MIT 라이선스, 74.3% 품질, 4.9초/문서, 100% 안정성, 멀티포맷 지원을 한꺼번에 갖춘 파서는 현재로선 Docling 정도다.

MinerU(AGPL)가 3%p 더 높은 품질을 제공하지만 14배 느리고 상용 제약이 있다. PyMuPDF4LLM(AGPL)은 2.5배 빠르지만 역시 AGPL이다. 라이선스 제약이 중요한 프로젝트라면 Docling을 우선 검토할 만하다.
