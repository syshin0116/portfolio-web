...
variable "agent_secret_versions" {
  description = "Reviewed Secret Manager numeric versions keyed by all twelve managed secret IDs; null only during foundation bootstrap. Secret payloads never enter Terraform."
  type        = map(string)
  default     = null
  ...
  validation {
    ...
    error_message = "agent_secret_versions must contain exactly the twelve managed secret IDs, with no missing or extra keys."
  }
}
