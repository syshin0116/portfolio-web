...
그래서 "단일 진실 공급원" 대신 **"단일 종합 공급원(single source of synthesis)"**이 맞다. 중앙은 진실을 *보관*하지 않고 *종합*한다. 한 프로젝트에서는 이걸 아주 물리적으로 지켰다. 회의록 본문은 원본 협업 도구에 남기고, 저장소에는 포인터와 백링크만 뒀다. 결정 노트에 이유를 짧게 적었는데, 요지는 사본은 늙는다는 것이었다. 중앙이 무언가를 *보관*하기 시작하는 순간 그 사본은 원본보다 먼저 stale 해지고, 그래서 중앙은 콘텐츠가 아니라 인덱스만 들고 있어야 한다. 단일 진실 공급원도 물리·연합·가상의 형태가 있지만, 조직 프로세스와 거버넌스 없이 그 자체를 위해 만들면 "비싸기만 하고 쓸모없다."[^ssot]

솔직히 적자면, 내가 통과한 한 프로젝트에서는 정반대 결정을 내렸다. 단일 팀이 소유하는 한 저장소 안에서는 문서 디렉터리를 정본으로 못 박았고, 코드와 문서가 충돌하면 문서가 이긴다는 중앙집중 규칙을 그대로 박았다. 결정자가 한 명이고 소유 경계가 없는 곳에서 federation은 그냥 과잉설계였다. 그러니 federation이 SSOT를 *대체*한다는 식으로 읽으면 곤란하다. 깨지는 축은 팀 대 조직이 아니라 소유권 경계다. 하나의 소유 도메인 안에서는 중앙 SSOT가 맞고, federation은 그 경계를 넘는 순간에 비로소 값을 한다.
...
내가 본 한 프로젝트도 같은 방향으로 결정했다(아직 구현 전이다). 에이전트의 모든 recall을 협업 단위 단위로 스코프 태깅하고, 세션 연속성과 세션 복구를 다른 저장소에 다른 책임으로 분리한다는 방향이었다. 둘을 섞으면 거짓 복구가 난다고 결정 노트에 적어 뒀다. 다만 메모리 substrate를 도구가 거저 주지는 않더라. 에이전트를 거치지 않은 사람의 행동은 자동으로 기록되지 않아서, 그 맥락은 도메인이 직접 밀어 넣어야 했다. "저장소에 있겠지"라는 가정이 가장 위험한 staleness였다.
...
- 다만 이 글의 이상을 내 프로젝트가 다 지켰느냐고 물으면, 아니다. 어떤 프로젝트에서는 깨끗한 federation 친화 설계를 포기하고 기존 스키마와 제약을 그대로 상속했다. 결정 노트에 "솔직히"라는 단어까지 넣어 트레이드오프로 남겼다. 범위가 작고 마감이 빠를 때는 federation의 이상보다 기존 중앙 자산을 재사용하는 쪽이 이긴다. federation은 옳은 방향이지만, 항상 옳은 선택은 아니다.
