...
summary: "Clidex v0.2를 88개 인기 CLI 도구로 실전 테스트한 결과 커버리지 43%, 검색 정확도 47%라는 낮은 수치가 나왔다. 원인을 분석하고, 데이터 소스 확장(Homebrew 자동 발견, Cask, npm), 검색 알고리즘 개선(쿼리 전처리, popularity boost, nucleo-matcher), 시맨틱 검색(Model2Vec + RRF) 도입으로 커버리지 91%, 정확도 82%를 달성했다. 또한 이전 인덱스를 stars 캐시로 재사용하고 GitHub API 호출을 병렬화하여 일일 빌드를 23분에서 5분으로 단축한 과정을 기록한다."
published: 2026-03-25
---

> [!summary]
> [이전 글](/projects/clidex/01-overview)에서 BM25 기반 CLI 도구 검색기 Clidex를 만들고, 20개 모의 도구 기준 100% recall을 달성했다. 하지만 실제 인기 CLI 도구 88개로 테스트하자 **커버리지 43%, 검색 정확도 47%**라는 현실이 드러났다. 이 글은 문제를 진단하고, 데이터 파이프라인과 검색 알고리즘을 전면 개편하여 **커버리지 91%, 정확도 82%**로 개선한 과정을 기록한다.
