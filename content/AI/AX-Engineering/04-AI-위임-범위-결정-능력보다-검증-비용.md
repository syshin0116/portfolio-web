...
- AI 능력의 경계(jagged frontier)는 들쭉날쭉하고 보이지 않아 **직관으로 미리 정할 수 없다.** 손으로 찔러봐야 안다.
- 2026년의 재정의: **검증할 수 없으면 위임하지 마라.** 위임의 단위는 곧 검증의 단위다.
...
경계가 보이지 않을수록 사이보그처럼 자주 확인하며 가는 편이 안전하다. 그리고 여기서 가장 위험한 함정이 *자동화 편향*이다. 자동화가 믿을 만할수록 사람은 감시를 놓아 버린다. 그래서 "human-in-the-loop"라는 말만으로는 안전장치가 되지 못한다.[^bias]

---

## 검증할 수 없으면 위임하지 마라

2026년 들어 이 문제를 실무로 끌어내린 재정의가 나왔다. Google DeepMind의 "Intelligent AI Delegation"은 **분해(decomposition)와 위임(delegation)을 구분**한다. 분해는 과제를 쪼개는 것이고, 위임은 권한과 책임을 넘기는 것이다. 그리고 핵심 규칙은 *contract-first decomposition*이다. **각 하위 과제의 출력이 검증 가능해질 때까지 재귀적으로 쪼개고, 주관성이 낮고 되돌리기 쉽고 성공 기준이 분명한 것만 AI에 위임한다.** 속도나 비용이 아니라.[^deepmind]
...
- **검증할 수 없으면 위임하지 마라.** 검증 가능할 때까지 분해하고(contract-first), 검증 비용·가역성·주관성 세 축으로 판정한다. 코드가 싸질수록 병목은 검증으로 옮겨간다.
