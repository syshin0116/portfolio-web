...
> [!note] 설계 철학
> Multi-Query 검색으로 많은 검색 결과를 얻을 수 있지만, 결과가 많다고 항상 좋은 것은 아니다. 중복되거나 관련성이 낮은 내용이 답변 생성 LLM에 전달되면 오히려 답변 품질이 저하될 수 있다. 그래서 엄격한 필터링으로 정말 필요한 정보만 추려서 전달하는 방식을 택했다.
...
학술 논문의 구조적 특성과 연구 정보 검색의 일반적 요구사항을 고려하여 시스템을 설계했다. 키워드 매칭에 머무르지 않고 연구 질문의 의도를 이해해, 관련된 다양한 정보를 종합적으로 제공하는 것을 목표로 했다.


## 결론

학술 논문 특화 파서 시스템은 PDF 변환을 출발점으로 삼아 **지능형 학술 정보 검색 플랫폼**까지 구현한 프로젝트다. Upstage Document Parse와 LLM의 결합, Multi-Query + Reranker 기반 하이브리드 검색, Parent-Child Document 구조 등 최신 AI 기술을 통합하여 학술 정보 검색의 정확성과 효율성을 극대화했다.
