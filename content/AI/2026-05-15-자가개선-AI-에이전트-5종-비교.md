...
> [!tip] 핵심 통찰
> OpenClaw → Hermes 이주 흐름의 이유는 대체로 하나다. OpenClaw는 매번 새 문제로 푸는데 Hermes는 경험을 스킬로 굳힌다는 점이다. 다만 그 자가 개선 루프의 약한 고리는 *자기 평가*다. 에이전트가 자기 출력을 정확히 못 보면, 학습된 스킬에 오류가 그대로 박힌다. 그래서 2026년 자가 개선 에이전트의 관건은 학습량보다도 자기 출력을 얼마나 정직하게 평가하느냐 쪽에 가까워 보인다.
