...
이 질문들에 정답은 아직 없다. 하지만 방향은 분명하다. **불확실할수록 산출물을 선 아래로 내려둔다.**
...
그리고 이게 시리즈 전체의 닫는 말이기도 하다. 8편을 관통한 한 줄은 이것이었다. **마법처럼 보이는 AI를, 도구가 바뀌어도 남는 엔지니어링된 지식 시스템으로 바꾸는 법.** 고객이 뭘 원하는지 몰라도(1~3편), 뭘 위임할지 정하고(4편), 데이터를 갖추고(5편), 구현의 빈틈을 결정으로 흡수하고(6편), 개인에서 조직으로 합치고(7편), 도구가 바뀌어도 살아남게(8편) 한다. 1편의 JTBD가 말했듯 **job은 안정적이고 구현은 휘발적이다.** Elicitation 산출물은 컴파일러의 입력이고, 도구는 컴파일러일 뿐이다.
