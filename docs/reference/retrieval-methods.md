...
336 markdown files, ~5.3 MB, Korean-language technical writing with heavy English
loanwords and code. YAML frontmatter (title, date, tags, categories). **A `[[wikilink]]`
graph between posts.**

Three things make this corpus worth evaluating on, in the order measurement says they
matter:

1. **174 aliased `[[target|alias]]` links.** The alias is the author's own Korean surface
   form for a target document - free known-item ground truth that no public benchmark
   corpus has. See [below](#aliased-wikilinks---free-known-item-ground-truth).
...
> **Measured, and weaker than assumed.** The graph over the live corpus:
> **123 of 336 files have any edge, 213 are isolated (63%)**, and components are
> `[95, 11, 4, 3, 3, 3, 2, 2]`. The `wiki/index.md` hub is degree **29**, not the ~100 an
> earlier pass estimated. Fixing the link parser barely helps: stripping code fences
> changes the edge count by **zero**, and path-form resolution adds **8** edges (227 to
> 235). Neither changes node coverage at all.
>
...
| Link-weighted reranking (PageRank-ish) | `planned` | Whether "well-connected" predicts "relevant". With 8 components and a 95-node giant, expect this to be mostly a prior on one cluster |
...
The corpus contains **174 aliased links** of the form `[[target|alias]]`, where the alias
is **the author's own Korean surface form for a target document**. That is a labelled
query-to-document pair, written by the person who knows the corpus best, for free, in
quantity - and **no public benchmark corpus has this.**
...
| Method | Status | Meant to teach |
|---|---|---|
| Alias-derived known-item query set | `planned` | 174 free gold pairs to bootstrap the eval dataset before any LLM-generated queries |
...
1. **A user dictionary.** `add_user_word("도커", "NNP")` restores `['도커']`, verified.
   Build it at build time from the corpus - **frontmatter `tags` are the best source**,
   since the author has already hand-labelled the domain vocabulary - plus code-block
   identifiers and English proper nouns.
...
3. **Index the surface form alongside morphemes**, so a term the dictionary has not caught
   up with still matches exactly instead of silently becoming a different word.
...
**Measured effect of the fix** on the live corpus, ground truth being files that literally
contain the term:

| | `도커` recall@13 | macro recall@10 |
|---|---|---|
| current | **0 / 13** | 0.323 |
| fixed | **13 / 13** | 0.605 |

The score normalisation is a separate fix with its own measurement: raw BM25 scores a real
query at 7.38 and a nonsense query at 4.13, while `score / max(scores)` flattens **both to
exactly 1.000**. Do not normalise inside a retriever - `rank` is authoritative and `score`
stays method-native.
