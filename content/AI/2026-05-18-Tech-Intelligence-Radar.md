...
draft: false
enableToc: true
description: 레포, 논문, 뉴스, 블로그, 소셜 신호를 자동으로 수집하고 계속 업데이트되는 지식으로 관리하기 위한 기술 정보 레이더 설계안.
summary: "기술 변화가 가속화되면서 사람이 모든 레포, 논문, 뉴스, changelog를 직접 따라가기는 어렵다. 이 글은 GitHub, 논문, 뉴스, 블로그, 소셜 신호를 별도 intelligence feed로 수집하고, 변화 감지와 재평가를 통해 필요한 내용만 지식으로 승격하는 구조를 설명한다."
...
| Layer | 역할 | 수명 | 예시 |
|---|---|---:|---|
| Raw signals | 외부에서 들어온 원본 신호 | 짧음 | HN 글, 새 release, 논문 링크, LinkedIn post |
| Intelligence feed | 사용자에게 보여줄 요약/추천 | 짧음~중간 | 오늘의 alert, 주간 digest, 이번 주 spike 후보 |
| Action queue | 실제 판단과 실험으로 이어지는 목록 | 중간 | spike, adopt 검토, reference note |
| Knowledge base | 검증된 장기 지식 | 김 | pattern, comparison, playbook, ADR, 실험 결과 |

대부분의 signal은 digest에서 소비되고 사라져도 된다. 지식으로 승격하는 기준은 훨씬 엄격해야 한다.
...
CREATE TABLE sources (
  ...
  type TEXT NOT NULL,              -- github, arxiv, news, blog, social, internal
  name TEXT NOT NULL,
  url TEXT,
...
CREATE TABLE items (
  id INTEGER PRIMARY KEY,
  source_id INTEGER,
  item_type TEXT NOT NULL,         -- repo, paper, news, blog_post, social_post, model
...
CREATE TABLE change_events (
  id INTEGER PRIMARY KEY,
  item_id INTEGER NOT NULL,
  event_type TEXT NOT NULL,        -- release, changelog, paper_update, news_mention
  event_at TEXT,
  title TEXT,
  body TEXT,
  summary TEXT,
...
CREATE TABLE assessments (
  ...
  next_action TEXT,                -- spike, adopt, reference, monitor, ignore, promote_to_knowledge
  rationale TEXT,
  valid_until TEXT,
  superseded_by TEXT
);
...
- `items`는 “대상”이다. LangGraph, 특정 논문, 특정 뉴스 글 같은 것.
- `observations`는 “시간에 따른 숫자/상태”다. star 수, 댓글 수, 최신 release 같은 것.
- `change_events`는 “변화 사건”이다. release, changelog, 논문 업데이트, 큰 뉴스 언급 같은 것.
...
| Actionability | 지금 spike/adopt/reference 중 하나로 이어질 수 있는가? |
...
지금 필요한 것은 “좋은 레포 목록”이 아니다. 레포, 논문, 뉴스, 블로그, 소셜, 내부 실험까지 포함해서 **계속 변하는 기술 정보를 추적하고 재평가하는 시스템**이다.
