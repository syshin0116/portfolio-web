...
summary: "동일한 PDF(Attention Is All You Need, 15p)를 5개 파서로 파싱하여 헤딩, 테이블, 수식, 이미지를 직접 비교한다. **Marker가 유일하게 헤딩 레벨을 구분**하고, **MinerU는 테이블/수식/이미지 모두 지원**한다. Docling은 깔끔하지만 수식 미지원, PyMuPDF4LLM은 빠르지만 구조 보존이 약하다."
