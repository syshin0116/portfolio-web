...
summary: "이전 글에서 커버리지 91%, 정확도 82%를 달성했지만, 검색 알고리즘에 구조적 모순이 남아 있었다. 오타 교정이 실질적으로 작동하지 않고, synonym 확장 결과가 뒷단 gate에서 죽고, intent coverage가 부분 문자열 매칭으로 오작동했다. 이 글은 이 세 가지 버그를 수정하고, 20개 fixture 기반 테스트를 28개 adversarial fixture + 5,277개 실제 인덱스 테스트로 강화하고, crates.io category discovery와 PyPI/pipx 데이터 소스를 추가한 과정을 기록한다."
published: 2026-04-04
---

> [!summary]
> [이전 글](/projects/clidex/02-search-improvement)에서 데이터 파이프라인을 확장해 커버리지 91%, 정확도 82%를 달성했다. 하지만 코드 리뷰에서 검색 알고리즘에 **구조적 모순 3가지**가 발견됐고, 테스트가 **20개 도구 fixture에서 100% 통과**하는 것이 실제 품질을 보장하지 못한다는 문제가 드러났다. 이 글은 알고리즘 버그 수정, 테스트 정교화, 데이터 소스 확장을 기록한다.
...
`expand_query()`로 synonym을 확장해 BM25 후보를 찾지만, 이후 confidence gate가 **원래 query term만** 체크했다. "manual pages"로 검색하면 synonym "man" → "tldr"로 확장되어 BM25가 tldr을 찾지만, gate에서 "manual"과 "pages"가 tldr 메타데이터에 없으므로 `covered == 0`으로 탈락했다.
...
`searchable.contains(&tl)`로 전체 메타데이터를 substring 검색하고 있었다. "at"이 "data"에 매칭되고, 빈 문자열이 모든 것에 매칭되는 등 우연한 부분 일치가 coverage로 잡혔다.
...
### 해결: Levenshtein distance + 다층 bailout
...
5,277개 도구에서 edit distance를 전부 계산하면 느려지므로 3개 bailout을 적용:
...
| 약점 | 설명 |
|---|---|
| **Fixture가 너무 작음** | 20개 도구에서 "top 5 포함"은 사실상 "결과에 있기만 하면 통과" |
...
| 검증 | 기준 |
|---|---|
| Must-have 도구 | 65개 중 63개 존재 (2개 known missing) |
| 카테고리 breadth | 최소 10개 top-level, 현재 83개 |
| 생태계 presence | Brew 500+, Cargo 30+, npm 10+ |
| 총 도구 수 | 최소 3,000개 (regression guard) |

### Adversarial fixture가 잡아낸 것들

테스트를 강화하자마자 실제 문제가 드러났다:

1. **"make http requests" → curlie가 httpie를 이김** — curlie의 description에 "curl"과 "httpie" 모두 포함되어 BM25 점수가 높음. 이건 genuinely debatable한 랭킹이라 강제 순서 대신 둘 다 top 3 확인으로 변경.
...
| **Synonym-only 매치** | gate에서 탈락 | manual pages→tldr, navigate quickly→zoxide |
| **실제 인덱스 테스트** | 없음 | **10/10** (5,277 도구) |
| **Coverage 테스트** | 없음 | **65/65** must-have (2 known missing) |
...
20개 도구에서 100% recall은 의미 없었다. 경쟁 도구가 없으면 랭킹 테스트가 아니라 존재성 테스트일 뿐이다. Adversarial fixture를 추가하자마자 `"make http requests"` 같은 genuinely debatable한 랭킹 문제가 드러났다.
...
"jq가 인덱스에 있는가?"를 검색 테스트로 간접적으로 검증하는 것보다, "65개 must-have 도구가 모두 존재하는가?"를 직접 검증하는 게 훨씬 명확하다. 데이터 소스가 깨졌을 때 바로 잡아낸다.

### 4. Substring 매칭은 함정이다

`searchable.contains(&tl)`은 직관적이지만, 빈 문자열이 모든 것에 매칭되고 "at"이 "data"에 매칭되는 등 예측 불가능한 동작을 만든다. 토큰 경계 기반 매칭으로 바꾸자 scoring 규칙이 일관적이 되었다.
...
- **crates.io/PyPI discovery 실행** — 다음 인덱스 빌드에서 실제 효과 측정
