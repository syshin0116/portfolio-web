...
Vitest는 테스트 파일을 어떤 단위로 격리할지 pool로 고른다. 기본값인 forks는 파일마다 새 process 환경을 만들고, 그때마다 React, testing-library, setup을 다시 import한다. 작은 파일 하나도 import에 1초가 들었다. 파일이 650개를 넘으니 이 고정비가 실제 assertion 시간보다 커졌다.

vmThreads는 worker process를 유지한 채 파일마다 새 VM context만 만든다. 공통 module 준비 비용을 줄이면서 파일별 window는 새로 받을 수 있었다. 로컬 shard wall은 64~80초에서 21~27초가 됐고, 실제 CI test step 중앙은 294초에서 90초가 됐다.
...
HTTP client module을 테스트마다 vi.mock하는 대신 MSW(Mock Service Worker)로 network request와 response를 가로챘다. production hook, react-query, fetch 경로는 실제 코드를 사용하고 외부 경계만 가짜로 만들었다.

이 작업은 테스트 파일 단위가 아니라 module 단위로 해야 했다. 같은 client module을 mock하는 파일이 하나라도 남으면 그 consumer cache가 다른 테스트에 영향을 줄 수 있었다. HR, opportunity, evaluation, analysis, generated client처럼 module 묶음별로 옮기며 매번 전체 suite를 실행했다.
...
- module mock 예외 테스트는 resetModules 뒤 대상 module을 동적 import
...
세 검증을 static job 하나로 합치고, 첫 step이 실패해도 뒤 검증은 취소되지 않도록 각 step 조건을 유지했다. 테스트 실행시간은 거의 같았지만 shard queue가 16~18초로 줄었다. code가 빨라진 것이 아니라 중요한 job이 먼저 runner를 잡게 된 효과였다.
...
마지막으로 API 테스트의 system time을 봤다. 모든 테스트가 끝날 때마다 clean_db fixture가 185개 table에 DELETE를 실행하고 있었다.
...
### 빈 table을 지우는데 왜 25ms가 드나

처음에는 이 코드가 비싸 보이지 않았다. 대부분의 table은 비어 있고, 빈 table의 DELETE는 SQLite가 즉시 끝낸다. 실제로 SQL 자체는 거의 공짜다.

비용은 SQL이 아니라 **문장을 보내는 왕복**에 있었다. SQLite에는 async driver가 없다. aiosqlite는 별도 thread에서 동기 sqlite3를 돌리고 event loop와 queue로 주고받는 wrapper다. 그래서 `await conn.execute(...)` 한 번마다 이렇게 움직인다.

    event loop ──put──> queue ──> worker thread
                                      sqlite3 실행
    event loop <──wake── socket <──put── 결과

thread가 결과를 넣고 event loop를 깨울 때 self-pipe socket에 쓰고, loop는 그걸 받으려고 selector에서 깨어난다. 문장 하나에 context switch 두 번과 socket 왕복 한 번이다. 185번이면 그게 185세트다.
...
테스트당 약 2,000회 loop wakeup이다. 그중 370회 정도가 cleanup의 DELETE 185개와 그 응답이었다. 벤치마크로 떼어내 재보니 정확히 맞았다.

    185개 DELETE 루프   25.0 ms / test

호출 횟수가 비용인 구조라 table을 몇 개 지우느냐가 곧 시간이었다. 그리고 대부분의 테스트는 185개 중 서너 개만 건드린다.

### 계산만 하는 테스트까지 cleanup을 낸 이유
...
그리고 cleanup은 `integration` marker가 붙었으면 수행했다. `tests/infra/` 아래는 순수 함수를 검증하든 뭘 하든 전부 대상이 됐다. 실제로 함수점수 계산만 하는 파일 하나가 이랬다.
...
marker로 대상을 좁히는 대신, SQLAlchemy Engine event로 어떤 table에 write가 갔는지 기록했다. cursor에 실제로 나간 문장을 보므로 ORM이든 raw SQL이든 다 잡힌다.
...
마지막 줄이 `clear()`가 아닌 이유가 있다. cleanup이 도는 중에 write가 더 들어오면, 전체를 비울 경우 그 write가 추적에서 사라져 다음 테스트로 샌다. snapshot만 차집합으로 빼면 그 사이 들어온 이름은 남아서 다음 teardown이 지운다.

marker 게이트는 함께 지웠다. write가 있으면 무조건 지우므로 marker를 안 붙여서 생기던 잔류 오염 경로도 같이 닫혔다. 전보다 좁게 지우는데 격리는 오히려 더 안전해졌다.

테스트를 병렬로 돌릴 때 쓰는 pytest-xdist는 worker마다 별도 process를 띄운다. 그래서 이 set은 worker별로 자연히 격리되고, 테스트 DB 파일 경로에도 PID가 들어가 있어 서로 섞이지 않는다.

### 이 변경이 감수한 것

전량 DELETE는 멍청하지만 완전했다. 어떤 경로로 write가 들어오든 다음 테스트는 빈 DB에서 시작한다. 새 코드는 문장을 정규식으로 읽는 heuristic이라, 못 알아보는 write 형태가 생기면 그 row가 살아남는다.

지금 코드베이스에서 나가는 write는 전부 잡히는 걸 확인했지만, 앞으로 누가 CTE를 앞세운 DML이나 `REPLACE INTO`, schema를 붙인 `INSERT INTO main.foo`를 쓰면 조용히 새기 시작한다. 실패 방식도 나쁘다. 그 테스트가 아니라 다음 테스트가, 그것도 실행 순서에 따라서만 깨진다.
...
전체 suite는 38% 빨라졌다. 기본 순서와 별도 random seed를 모두 실행해 이전 테스트의 data가 남지 않는 것도 확인했다.
...
테스트가 전부 통과한 뒤 cleanup에서 runner가 사라진 적이 있었다. 테스트 로그만 보면 cleanup 실패였지만 같은 시각 cluster autoscaler의 ScaleDown event가 있었다.
