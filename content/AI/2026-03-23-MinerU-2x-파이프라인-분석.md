...
테이블은 HTML `<table>` 태그로 변환된다. `rowspan`, `colspan` 등 복잡한 구조도 처리한다.

**Table 1 (단순 테이블)** — 원본 vs 변환 결과:

```html
<table>
  <tr>
    <td>Dataset</td><td># Documents</td><td># Avg. Pages</td>
    <td># Avg. Tokens</td><td># Doc Types</td><td># Questions</td>
  </tr>
  <tr>
    <td>DocBench</td><td>229</td><td>66</td>
    <td>46377</td><td></td><td>1102</td>
  </tr>
  <tr>
    <td>MMLongBench</td><td>135</td><td>47.5</td>
    <td>21214</td><td>5</td><td>1082</td>
  </tr>
</table>
```
...
<table>
  <tr>
    ...
    <td colspan="5">Domains</td>
    <td colspan="3">Types</td>
    <td rowspan="2">Overall</td>
  </tr>
  <tr>
    <td>Aca.</td><td>Fin.</td><td>Gov.</td><td>Law.</td><td>News</td>
    <td>Txt.</td><td>Mm.</td><td>Una.</td>
  </tr>
  <tr>
    <td>GPT-4o-mini</td><td>40.3</td><td>46.9</td><td>60.3</td>
    <td>59.2</td><td>61.0</td><td>61.0</td><td>43.8</td><td>49.6</td><td>51.2</td>
  </tr>
  <!-- ... -->
</table>
```

`rowspan="2"`와 `colspan="5"` 같은 복잡한 셀 병합도 정확하게 처리한다. 테이블 캡션(`Table 1: Statistics of...`)도 별도로 추출된다.

**주의점**: CPU와 MPS에서 테이블 OCR 결과에 미세한 차이가 있다. 예를 들어 CPU에서는 `GPT-4o-mini`로 인식한 셀이 MPS에서는 `GPT-40-mini`로 인식되는 경우가 있었다.

### 수식 처리

인라인 수식과 디스플레이 수식 모두 LaTeX로 변환된다:

**인라인 수식** (텍스트에 포함):
```
each knowledge source $k _ { i } \in \mathcal { K }$
```

**디스플레이 수식** (독립 블록):
```latex
$$
k _ { i } { \xrightarrow { \mathrm { \tiny ~ D e c o m p o s e } } }
\{ c _ { j } = ( t _ { j } , x _ { j } ) \} _ { j = 1 } ^ { n _ { i } } ,
$$
```
...
이미지는 `images/` 폴더에 JPG로 추출되고, Markdown에서 참조된다:

```markdown
![](images/5e96015...8b578f.jpg)
Figure 1: Overview of our proposed universal RAG framework RAG-Anything.
```

캡션은 이미지 바로 아래에 텍스트로 출력된다. 12개의 이미지가 추출되었으며, 파일명은 콘텐츠 해시값으로 생성된다.
