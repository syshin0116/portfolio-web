...
draft: false
enableToc: true
description: 학술 논문을 위한 전문 문서 파싱 및 RAG 시스템 구축 프로젝트 분석. Upstage Document Parse와 LLM을 활용한
  고품질 멀티모달 콘텐츠 추출, 벡터 데이터베이스 구축, 그리고 Multi-Query + Reranker 기반 하이브리드 검색 시스템 구현까지 포괄적으로
  다룬다.
summary: 특정 학회 논문 1,261개를 대상으로 한 전문 문서 파싱 및 RAG 시스템 구축 프로젝트다. Upstage Document Parse와
  LLM을 결합한 이중 파싱 파이프라인을 통해 멀티모달 콘텐츠(텍스트, 이미지, 테이블)를 정확하게 추출하고, LangChain MarkdownHeaderTextSplitter로
  의미적 청킹을 수행한다. 15,818개 벡터 포인트를 Qdrant에 저장하여 Parent-Child Document 구조를 구현했으며, Multi-Query
  + Reranker 기반 하이브리드 검색으로 검색 정확도를 극대화했다. 학술 논문 처리에 최적화된 완전 자동화 파이프라인으로 RAG 시스템 즉시
  활용이 가능하다.
...
> [!summary]
> 
> 특정 학회 논문 1,261개를 대상으로 한 전문 문서 파싱 및 RAG 시스템 구축 프로젝트다. Upstage Document Parse와 LLM을 결합한 이중 파싱 파이프라인을 통해 멀티모달 콘텐츠(텍스트, 이미지, 테이블)를 정확하게 추출하고, LangChain MarkdownHeaderTextSplitter로 의미적 청킹을 수행한다. 15,818개 벡터 포인트를 Qdrant에 저장하여 Parent-Child Document 구조를 구현했으며, Multi-Query + Reranker 기반 하이브리드 검색으로 검색 정확도를 극대화했다. 학술 논문 처리에 최적화된 완전 자동화 파이프라인으로 RAG 시스템 즉시 활용이 가능하다.
...
학술 논문 PDF를 구조화된 마크다운으로 변환하는 핵심 엔진이다. Upstage Document Parse의 강력한 레이아웃 분석 능력과 LLM의 텍스트 정제 기능을 결합하여 최고 품질의 파싱 결과를 달성한다.

**핵심 기능:**
- **이중 파싱 전략**: Upstage Document Parse 기본 추출 + LLM 후처리로 정확도 극대화
...
### Phase 1: Upstage Document Parse 기반 초기 파싱

```
PDF 입력 → Upstage Document Parse → 배치 처리 (50페이지 단위) → 멀티모달 추출
```

Upstage Document Parse를 활용한 1차 파싱으로 PDF의 기본 구조와 콘텐츠를 추출한다. 50페이지 단위 배치 처리를 통해 대용량 논문도 안정적으로 처리한다.
...
Upstage Document Parse 결과를 LLM으로 정제하여 파싱 정확도를 극대화한다. 15,000자 단위로 분할하여 처리하며, 학술 논문에 특화된 프롬프트를 사용한다.
...
- **우선순위 시스템**: LLM 추출 > 파일명 추출 > 기본값 순서로 적용
...
JSON으로 저장된 TOC 구조를 LLM에 전달할 때는 다음과 같이 `|_` tree 형태로 변환한다:
...
|_ 3. Experimental Results and Discussion
  |_ 3.1 Start of Injection (SoI)
  |_ 3.2 Dwell Time (Ignition Energy Release Period)
|_ 4. Conclusion
```

이 tree 구조를 통해 LLM은 현재 검색된 섹션이 논문 전체에서 어느 위치에 있는지 파악할 수 있다. 각 청크가 헤딩 기준으로 분할되어 있으므로 섹션 제목 자체가 위치 정보 역할을 하며, tree 구조와 결합하면 사용자에게 논문의 전체 맥락을 직관적으로 전달할 수 있다.
...
3. **위치 정보**: 헤딩 기준 청킹이므로 섹션 제목(예: "3.1 Start of Injection")이 곧 현재 위치를 나타냄
4. **시각적 구조**: `|_` 형태의 tree 구조로 논문 전체 구조를 직관적으로 표시
5. **탐색 가능성**: tree 구조를 통해 관련 섹션 (예: 3.2 Dwell Time)으로의 확장 검색 지원
...
### 개선 가능한 검색 기법: Multi-Vector Retriever + Hypothetical Questions

현재 시스템은 각 청크에 대해 단일 임베딩만 생성하여 검색에 활용하지만, **Multi-Vector Retriever** 기법을 적용하면 검색 품질을 한 단계 더 끌어올릴 수 있다.

**Multi-Vector Retriever란?**

하나의 청크에 대해 여러 종류의 벡터 표현을 생성하고 저장하는 방식이다. 검색 시에는 이 다양한 벡터들과 매칭하여 더 풍부한 검색 결과를 제공한다.

- **원본 텍스트 임베딩**: 현재 시스템과 동일한 기본 벡터
- **Hypothetical Questions (가상 질문)**: LLM이 각 청크를 읽고 "이 내용으로 답할 수 있는 질문"을 여러 개 생성하여 임베딩. 사용자의 실제 질문과 가상 질문 간의 유사도가 높아 검색 정확도가 크게 향상됨
- **요약 임베딩**: 각 청크의 핵심 내용을 요약한 벡터
- **태그/키워드 임베딩**: 논문의 키워드, 분류 태그 등 메타데이터를 별도로 벡터화

**예시:**
```
청크: "직접분사식 암모니아 엔진에서 분사 시기를 10° BTDC에서 30° BTDC로 변경하면..."

→ Hypothetical Questions:
  - "암모니아 엔진에서 분사 시기를 변경하면 연소 특성이 어떻게 달라지는가?"
  - "직접분사식 엔진의 최적 분사 시기는?"
  - "BTDC 각도에 따른 연소 효율 차이는?"

→ Tags: ["암모니아", "직접분사", "분사시기", "연소특성", "BTDC"]
```

이 기법을 적용하면 사용자가 어떤 형태로 질문하더라도 관련 청크를 더 정확하게 찾아낼 수 있으며, 특히 학술 논문처럼 전문 용어와 일반 용어가 혼재하는 도메인에서 큰 효과를 기대할 수 있다.
...
4. **하이브리드 검색**: Multi-Query + Reranker로 검색 정확도 및 관련성 대폭 향상
5. **멀티모달 검색 준비 완료**: 이미지와 테이블 description 생성 및 벡터화로 시각적 질의응답 시스템 구현 기반 마련
6. **확장 가능한 아키텍처**: 다른 학회나 도메인으로 쉽게 확장 가능

### **혁신적 접근**

- **이중 파싱 전략**: Upstage Document Parse + LLM 조합으로 단일 파서 한계 극복
...
학술 논문 특화 파서 시스템은 단순한 PDF 변환 도구를 넘어선 **지능형 학술 정보 검색 플랫폼**이다. Upstage Document Parse와 LLM의 결합, Multi-Query + Reranker 기반 하이브리드 검색, Parent-Child Document 구조 등 최신 AI 기술을 통합하여 학술 정보 검색의 정확성과 효율성을 극대화했다.
